...
f<-function(iterator){
  pb<-txtProgressBar(1, iterator, style = 3)
  count = 0
  function(...){
...
packages = c('dplyr', 'haven', 'mosaicCalc', 'maxLik', 'tidyr', 'stats4', 'utils')
#library(errorist)
setwd('C:/Users/Public/SowVallette')
getwd()
oo=0

packages = c('dplyr', 'haven', 'mosaicCalc', 'maxLik', 'tidyr', 'stats4')
...
msl <- function(param){
  ...
  Phi = function(t, a1 ,a2 ,a3 ,A =c(0,365,730, Inf)){##Use exponential as a constraint to have a positive baseline hazard
    exp(a1)*between(t, A[1], A[2])+
      exp(a2)*between(t, A[2]+1, A[3])+
      exp(a3)*between(t, A[3]+1, A[4])
  ...
  res<- foreach(indiv = 1:42, .combine = f(42), .packages = packages) %dopar%{ 
    #nrow(df)){

    #sink("log.txt", append = TRUE)
    print(indiv)
    df = read_stata(file = "./modified/data_df.dta")
    
    t_ue = as.numeric(df[indiv, 'duration_unemployment'])
    t_ut = as.numeric(df[indiv, 'un_pre_training'])
    t_tu = as.numeric(df[indiv, 'duration_tu'])
    ...
    log(l_eu_int(t=t_ue)*l_ut_int(t=t_ut)*l_tu_int(t=t_tu)+0.000001)
  }
  #stopCluster(cl)
...
msl_result <- maxLik(msl, start = c(##Related to UE####
  ...
  method = 'BHHH', 
  control=maxControl(iterlim = 1,printLevel = 3),  fixed = c(rep(F, 42)))
summary(msl_result)
